...
-- Список медиафайлов пользователей, указав название типа медиа (id, filename, name_type)
-- (используя JOIN с семинара с фио дополнительно)
SELECT 
	m.id,
	m.filename AS 'медиа',
	mt.name_type AS 'тип медиа',
...
SELECT CASE (p.gender)
	WHEN 'm' THEN 'Муж'
	WHEN 'f' THEN 'Жен'
    END AS 'Пол', count(l.id) AS 'Лайков'
    FROM profiles p
    JOIN likes l 
...
SELECT DISTINCT u.id, CONCAT(u.firstname, ' ', u.lastname) AS 'Не отправляли сообщения'
FROM users u
WHERE NOT EXISTS (
	SELECT from_user_id
	FROM messages m
